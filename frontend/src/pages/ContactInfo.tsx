...
// Add this after imports to detect demo mode
const USE_DEMO_MODE = true; // Set to true for demo environment
...
const getOfficeForRole = (role: string): string => {
  switch (role) {
    case "admin":
      return "Main Office";
    case "teacher":
...
const getOfficeHoursForRole = (role: string): string => {
  switch (role) {
    case "admin":
      return "Monday - Friday: 8:00 AM - 4:00 PM";
    case "teacher":
      return "Monday, Wednesday, Friday: 3:00 PM - 4:00 PM";
    case "librarian":
...
const ContactInfo = () => {
  ...
  useEffect(() => {
    const fetchUserInfo = async () => {
      ...
      try {
        ...
        const isDemoMode =
          USE_DEMO_MODE || (token && token.startsWith("demo_"));

        if (isDemoMode) {
          console.log("Using demo mode for contact info");
          // Set default contact info based on user role
          setContactInfo({
            ...
            office: getOfficeForRole(user.role),
            office_hours: getOfficeHoursForRole(user.role),
            role: user.role,
          });
          setIsLoading(false);
          return;
        ...
        console.log("Using demo data for contact info due to error");
    ...
    const fetchTeachers = async () => {
      ...
      try {
        ...
        if (isDemoMode) {
          console.log("Using demo mode for teachers list");
          ...
          setTeachers([
            ...
            },
            {
              id: 3,
              name: "Mark Johnson",
              email: "mark.johnson@eduzone.com",
              phone: "(555) 123-4569",
              department: "English",
              office: "Room 103",
              office_hours: "Monday, Wednesday: 1:00 PM - 2:00 PM",
              role: "teacher",
              subject: "Literature",
              position: "English Teacher",
              bio: "English teacher with a focus on classic literature and creative writing.",
              image: "/images/school-logo.png",
            },
          ]);
          return;
        ...
        console.log("Using mock data for teachers due to error");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Check if we're in demo mode
      const token = localStorage.getItem("eduzone-token");
      const isDemoMode = USE_DEMO_MODE || (token && token.startsWith("demo_"));
      ...
      if (isDemoMode) {
        ...
        setTimeout(() => {
          ...
          toast({
            ...
            description:
              "Your contact information has been successfully updated in demo mode.",
          });
          setIsSaving(false);
        }, 500);
        return;
  ...
  const handleAddTeacher = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isDemoMode) {
        // In demo mode, just simulate a successful add
        setTimeout(() => {
          ...
          toast({
            title: "Teacher Added",
            description:
              "The teacher has been successfully added in demo mode.",
          });
          setIsSaving(false);
        }, 500);
        return;
  ...
  const handleDeleteTeacher = async (id: number) => {
    try {
      ...
      if (isDemoMode) {
        // In demo mode, just simulate a successful delete
        setTeachers(teachers.filter((teacher) => teacher.id !== id));

        toast({
          title: "Teacher Deleted",
          description:
            "The teacher has been successfully removed in demo mode.",
        });
        return;
      }
