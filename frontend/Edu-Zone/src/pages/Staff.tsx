...
const Staff = () => {
  ...
  useEffect(() => {
    const fetchStaff = async () => {
      try {
        setLoading(true);
        const response = await api.get<{ data: { data: Staff[] } }>("/staff");
        ...
        if (
          ...
          const transformedStaff = response.data.data.data.map((member) => ({
        ...
        // If no API data, we'll keep using the mock data we initialized with
      } catch (error) {
        console.error("Error fetching staff:", error);
        // Keep using mock data on error (already set in initial state)
        toast({
